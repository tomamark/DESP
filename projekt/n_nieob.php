<?php
/*
 * Skrypt obsługujący Nieobecności w menu Nauczyciela
 */
...
try {//nawiazywanie polaczenia z baza i odczyt danych
    ...
    foreach ($stmt as $row){//generowanie listy klas
    ...
    if ((isset($_POST['id_klasy'])) or(isset($_SESSION['id_klasy'])) ){
       ...
       $stmt->execute();
       $klasa=$stmt->fetch(PDO::FETCH_ASSOC);
        $TRESC.=genNieobecnosciKlasy ($pdo, $id_klasy,$klasa['klasa']);
    }
    else{
        $TRESC.=genNieobecnosciKlasy ($pdo,$klasa['id_klasy'],$klasa['klasa']);
    }
    $stmt->closeCursor();
}
