<?php
/*
 * Skrypt obsługujący Wysyłanie wiadomości w menu Nauczyciela
 */
...
try {//nawiazywanie polaczenia z baza i odczyt danych
    ...
    if ((isset($_POST['id_klasy'])) or(isset($_SESSION['id_klasy'])) ){
        ...
        $stmt->execute();
        $klasa=$stmt->fetch(PDO::FETCH_ASSOC);
        $TRESC.=genWiadomoscKlasy($pdo, $id_klasy, $klasa['klasa'], $powrot);
    }
    else{
        $TRESC.=genWiadomoscKlasy ($pdo,$klasa['id_klasy'],$klasa['klasa'], $powrot);
    }
    $stmt->closeCursor();
}
catch (PDOException $e){//nie mozan polaczec sie z BD
