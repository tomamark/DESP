<?php
/*
 * Skrypt obsługujący Oceny w menu Nauczyciela
 */
...
try {//nawiazywanie polaczenia z baza i odczyt danych
        ...
        if ((isset($_POST['id_przedmiotu'])) or(isset($_SESSION['id_przedmiotu'])) ){
            ...
            $stmt->execute();
            $przedmiot=$stmt->fetch(PDO::FETCH_ASSOC);
            $TRESC.=genOcenyPrzedmiotuKlasy ($pdo, $id_przedmiotuklasy,$przedmiot['id_klasy'],$przedmiot['klasa'],$przedmiot['przedmiot']);
        }
        else{
            $TRESC.=genOcenyPrzedmiotuKlasy ($pdo, $przedmiot['id_przedmiotuklasy'],$przedmiot['id_klasy'],$przedmiot['klasa'],$przedmiot['przedmiot']);
        }
        $stmt->closeCursor();
    }
